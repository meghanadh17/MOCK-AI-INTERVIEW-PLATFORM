...
class RecentSessionAdapter(
...
) : ListAdapter<InterviewSession, RecentSessionAdapter.ViewHolder>(DiffCallback) {
    ...
    inner class ViewHolder(val binding: ItemSessionCardBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: InterviewSession) {
            ...
            binding.tvDetail.text = "${item.type.uppercase()} · ${item.status.uppercase()} · $formattedDate"
            ...
            if (item.type.lowercase() == "video") {
                ...
                binding.ivIcon.imageTintList = ContextCompat.getColorStateList(binding.root.context, R.color.color_video)
            } else {
