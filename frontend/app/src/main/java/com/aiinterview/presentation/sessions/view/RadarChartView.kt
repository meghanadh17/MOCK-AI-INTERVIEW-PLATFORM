...
class RadarChartView @JvmOverloads constructor(
    ...
    private val labels = arrayOf("Technical", "Communication", "Confidence", "Structure", "Relevance")
    ...
    private val dataStrokePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = 0xFFF59E0B.toInt() // Orange/Amber
        style = Paint.Style.STROKE
        strokeWidth = 4f
    }

    private val dataFillPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = 0x33F59E0B.toInt() // 20% opacity Orange/Amber
    ...
    private val labelPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = ContextCompat.getColor(context, R.color.text_sec)
        textSize = 28f
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val maxRadius = minOf(width, height) * 0.32f
        ...
        for (i in 0..4) {
            ...
            val labelX = centerX + (maxRadius + 40f) * cos(angle).toFloat()
            val labelY = centerY + (maxRadius + 30f) * sin(angle).toFloat() + 8f
